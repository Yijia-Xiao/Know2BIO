...
EUC_MODELS = ["TransD", "TransE", "TransH", "TransR", "DistMult", "RESCAL", "CP", "MurE", "RotE", "RefE", "AttE"]
...
class TransR(BaseE):
    """TransR model with entity and relation projections"""

    def __init__(self, args):
        super(TransR, self).__init__(args)
        self.sim = "dist"
        self.ent_proj = nn.Linear(self.rank, self.rank)

    def get_queries(self, queries):
        head_e = self.entity(queries[:, 0])
        rel_e = self.rel(queries[:, 1])

        head_e = self.ent_proj(head_e)
    ...
    def get_rhs(self, queries, eval_mode):
        """Get embeddings and biases of target entities."""
        if eval_mode:
            # print(self.ent_proj(self.entity.weight).shape)
            return self.ent_proj(self.entity.weight), self.bt.weight
        else:
            return self.ent_proj(self.entity(queries[:, 2])), self.bt(queries[:, 2])
