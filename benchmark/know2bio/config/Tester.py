...
class Tester(object):
    ...
    def run_link_prediction(self, type_constrain = False):
        ...
        # print (hit10)
        print('mrr, mr, hit10, hit3, hit1', mrr, mr, hit10, hit3, hit1)
    ...
    def run_triple_classification(self, threshlod = None):
        ...
        LABL = [bool(i) for i in ans]
        PRED = [bool(i <= threshlod) for i in score]

        def calculate_metrics(PRED, LABL):
            # Ensure PRED and LABL have the same length
            assert len(PRED) == len(LABL), "Arrays PRED and LABL must have the same length."
            # Initialize counters
            TP = 0
            TN = 0
            FP = 0
            ...
            for i in range(len(PRED)):
                if PRED[i] and LABL[i]:  # True positive
                    TP += 1
                elif not PRED[i] and not LABL[i]:  # True negative
                    TN += 1
                elif PRED[i] and not LABL[i]:  # False positive
                    FP += 1
                elif not PRED[i] and LABL[i]:  # False negative
                    FN += 1
        ...
        def calculate_metrics_scores(PRED, LABL):
            TP, TN, FP, FN = calculate_metrics(PRED, LABL)
        ...
        accuracy, precision, recall, f1_score = calculate_metrics_scores(PRED, LABL)
        ...
        return acc, threshlod, accuracy, precision, recall, f1_score
