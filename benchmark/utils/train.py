...
def get_savedir(model, dataset, params):
    """Get unique saving directory name."""
    dt = datetime.datetime.now()
    date = dt.strftime("%m_%d")
    save_dir = os.path.join(
        './logs/', date, dataset,
        model + "_" + params # dt.strftime('_%H_%M_%S')
    )
...
def format_metrics(metrics, split):
    ...
    result = "\t {} MR: {:.6f} | ".format(split, metrics['MR'])
    result += "MRR: {:.6f} | ".format(metrics['MRR'])
    result += "H@1: {:.6f} | ".format(metrics['hits@[1,3,10]'][0])
    result += "H@3: {:.6f} | ".format(metrics['hits@[1,3,10]'][1])
    result += "H@10: {:.6f}".format(metrics['hits@[1,3,10]'][2])
    return result
